...
public class Ytake extends JFrame implements ActionListener{
	JTextField tfMoney;
	...
	BufferedReader br;
	PrintWriter pw;
	public Ytake(BufferedReader br,PrintWriter pw,Card currCard){
		this.br=br;
		this.pw=pw;
...
arg0.getSource()==bt006||arg0.getSource()==bt007||arg0.getSource()==bt008||arg0.getSource()==bt009){
			...
			if(takeMoney%100!=0)
				JOptionPane.showMessageDialog(Ytake.this,"对不起，请取整额");
			else{
			if(currCard.getMoney()<takeMoney){
				JOptionPane.showMessageDialog(Ytake.this, "对不起，您的余额不足！");
				tfMoney.setText("");
			}
			else{
				//发送新的账户信息给服务器
				...
				try {
					...
					if(strEcho.equals("Success")){
						...
						JOptionPane.showMessageDialog(Ytake.this, "定期六月存款成功");
					}else if("takeFailure".equals(strEcho)){
						JOptionPane.showMessageDialog(Ytake.this, "存款失败，请重试");
					}
